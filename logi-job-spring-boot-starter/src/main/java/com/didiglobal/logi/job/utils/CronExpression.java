 ...
 *
 *<P>Cron expressions are comprised of 6 required fields and one optional field
...
public final class CronExpression implements Serializable, Cloneable {
  ...
  public Date getNextInvalidTimeAfter(Date date) {
    ...
    // Performance will be very bad here, depending on the cron expression.
    ...
    try {
      ...
      while (exprsTok.hasMoreTokens() && exprOn <= YEAR) {
        ...
        if (exprOn == DAY_OF_MONTH && expr.indexOf('L') != -1 && expr.length() > 1
                && expr.contains(",")) {
          throw new ParseException("Support for specifying 'L' and 'LW' with other days of the "
                  + "month is not implemented", -1);
        }
        // throw an exception if L is used with other days of the week
        if (exprOn == DAY_OF_WEEK && expr.indexOf('L') != -1 && expr.length() > 1
                && expr.contains(",")) {
          throw new ParseException("Support for specifying 'L' with other days of the week is "
                  + "not implemented", -1);
        }
        if (exprOn == DAY_OF_WEEK && expr.indexOf('#') != -1
                && expr.indexOf('#', expr.indexOf('#') + 1) != -1) {
          throw new ParseException("Support for specifying multiple \"nth\" days is not "
                  + "implemented.", -1);
        }
      ...
      boolean dayOfM = !dom.contains(NO_SPEC);
      boolean dayOfW = !dow.contains(NO_SPEC);

      if (!dayOfM || dayOfW) {
        if (!dayOfW || dayOfM) {
          throw new ParseException("Support for specifying both a day-of-week AND a day-of-month "
  ...
  protected int checkNext(int pos, String s, int val, int type)
    ...
    if (c == 'W') {
      ...
      if (val > 31) {
        throw new ParseException("The 'W' option does not make sense with values larger than 31 "
                + "(max number of days in a month)", i);
      }
      TreeSet<Integer> set = getSet(type);
      set.add(val);
      i++;
  ...
  protected void addToSet(int val, int end, int incr, int type)
    ...
    if (stopAt < startAt) {
      switch (type) {
        ...
        case YEAR : {
          throw new IllegalArgumentException("Start year must be less than stopByJobCode year");
        }
        default : {
  ...
  public Date getTimeAfter(Date afterTime) {
    ...
    while (!gotOne) {
      ...
      // get day...................................................
      boolean dayOfM = !daysOfMonth.contains(NO_SPEC);
      boolean dayOfW = !daysOfWeek.contains(NO_SPEC);
      if (dayOfM && !dayOfW) { // get day by day of month rule
        ...
        if (lastdayOfMonth) {
          if (!nearestWeekday) {
            ...
            tcal.set(Calendar.DAY_OF_MONTH, day);
            tcal.set(Calendar.MONTH, mon - 1);
            Date time = tcal.getTime();
            if (time.before(afterTime)) {
              day = 1;
              mon++;
            }
          }
      ...
      } else if (dayOfW && !dayOfM) { // get day by day of week rule
        ...
        throw new UnsupportedOperationException("Support for specifying both a day-of-week AND a "
