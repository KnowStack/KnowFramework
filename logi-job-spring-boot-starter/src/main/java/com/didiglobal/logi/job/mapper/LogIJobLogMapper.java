...
public interface LogIJobLogMapper {
  ...
  @Insert("INSERT INTO logi_job_log(job_code, task_code, task_id, task_name, class_name, try_times, worker_code, worker_ip,"
          + "start_time, end_time, status, error, result, create_time, update_time, app_name) "
          + "VALUES(#{jobCode}, #{taskCode}, #{taskId}, #{taskName}, #{className}, #{tryTimes}, #{workerCode}, #{workerIp}, #{startTime}"
          + ", #{endTime}, #{status}, #{error}, #{result}, #{createTime}, #{updateTime}, #{appName})")
  int insert(LogIJobLogPO logIJobLogPO);

  @Select("select id, job_code, task_code, task_id, task_name, class_name, try_times, worker_code, worker_ip, start_time, "
  ...
  @Update("update logi_job_log set job_code=#{jobCode}, task_code=#{taskCode}, task_id=#{taskId}, task_name=#{taskName},  class_name=#{className}, try_times="
  ...
  List<LogIJobLogPO> pagineListByCondition(@Param("appName") String appName,
                                           @Param("taskId") Long taskId, @Param("taskName") String taskName,
  ...
  Integer pagineCountByCondition(@Param("appName") String appName, @Param("taskId") Long taskId,
                                 @Param("taskName") String taskName, @Param("jobStatus") Integer jobStatus,
                                 @Param("beginTime") Timestamp beginTime, @Param("endTime") Timestamp endTime);
}
