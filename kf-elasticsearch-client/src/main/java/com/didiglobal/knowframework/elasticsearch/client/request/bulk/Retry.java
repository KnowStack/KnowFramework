package com.didiglobal.knowframework.elasticsearch.client.request.bulk;
...
public class Retry {
    ...
    public void withAsyncBackoff(ESClient client, ESBatchRequest bulkRequest, ActionListener<ESBatchResponse> listener) {
        Retry.AsyncRetryHandler r = new Retry.AsyncRetryHandler(retryOnThrowable, backoffPolicy, client, listener);
    ...
    static class AbstractRetryHandler implements ActionListener<ESBatchResponse> {
        ...
        public void onResponse(ESBatchResponse bulkItemResponses) {
            if (!bulkItemResponses.hasFailures()) {
                ...
                addResponses(bulkItemResponses, Retry.TruePredicate.INSTANCE);
                finishHim();
            } else {
                if (canRetry(bulkItemResponses)) {
                    addResponses(bulkItemResponses, new Retry.BulkItemResponsePredicate() {
                        @Override
        ...
        private void retry(final ESBatchRequest bulkRequestForRetry) {
            ...
            scheduledRequestFuture = client.threadPool().schedule(next, ThreadPool.Names.SAME, new Runnable() {
                @Override
                public void run() {
                    Retry.AbstractRetryHandler.this.execute(bulkRequestForRetry);
                }
            });
        }
        ...
        private void addResponses(ESBatchResponse response, Retry.BulkItemResponsePredicate filter) {
    ...
    static class AsyncRetryHandler extends Retry.AbstractRetryHandler {
    ...
    static class SyncRetryHandler extends Retry.AbstractRetryHandler {
        private final PlainActionFuture<ESBatchResponse> actionFuture;

        public static Retry.SyncRetryHandler create(String retryOnThrowable, BackoffPolicy backoffPolicy, ESClient client) {
            PlainActionFuture<ESBatchResponse> actionFuture = PlainActionFuture.newFuture();
            return new Retry.SyncRetryHandler(retryOnThrowable, backoffPolicy, client, actionFuture);
        }
    ...
    private static class TruePredicate implements Retry.BulkItemResponsePredicate {
        private static final Retry.TruePredicate INSTANCE = new Retry.TruePredicate();
