drop table if exists `kf_task`;
CREATE TABLE `kf_task`
(
...
drop table if exists `kf_task_lock`;
CREATE TABLE `kf_task_lock`
(
    `id`          BIGINT(20) auto_increment,
    `task_code`   VARCHAR(100) DEFAULT '' NOT NULL COMMENT 'task taskCode',
...
drop table if exists `kf_job_log`;
CREATE TABLE `kf_job_log`
(
    `id`          BIGINT(20) auto_increment,
...
drop table if exists `kf_worker`;
CREATE TABLE `kf_worker`
(
    `id`              BIGINT(20) auto_increment,
    `worker_code`     VARCHAR(100) DEFAULT '' NOT NULL COMMENT 'worker taskCode',
...
drop table if exists `kf_worker_blacklist`;
CREATE TABLE `kf_worker_blacklist`
(
    `id`          BIGINT(20) auto_increment,
    `worker_code` VARCHAR(100) DEFAULT '' NOT NULL COMMENT 'worker taskCode',
    `create_time` DATETIME     DEFAULT NOW() COMMENT '创建时间',
