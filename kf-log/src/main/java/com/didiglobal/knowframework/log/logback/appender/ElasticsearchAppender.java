package com.didiglobal.knowframework.log.logback.appender;
...
public class ElasticsearchAppender extends UnsynchronizedAppenderBase<ILoggingEvent> {
	...
	public String getExtendsMappingClass() {
		return extendsMappingClass;
	}

	public void setExtendsMappingClass(String extendsMappingClass) {
	...
	public Boolean getDiscardWhenBufferIsFull() {
		return discardWhenBufferIsFull;
	}

	public void setDiscardWhenBufferIsFull(Boolean discardWhenBufferIsFull) {
		this.discardWhenBufferIsFull = discardWhenBufferIsFull;
	}
	...
	protected void append(ILoggingEvent event) {
		Map<String, Object> items = logEvent2Map(event);
		processAndPutLogEventInBuffer(items);
	}
	...
	private void processAndPutLogEventInBuffer(Map<String, Object> element) {
		if(null != element) {
			while (true) {
	...
	void putMetric(Metric metric, Map<String, Object> item) {
	...
	void putSpan(Span span, Map<String, Object> item) {
	...
	void putLog(Log log, Map<String, Object> item) {
	...
	private Map<String, Object> logEvent2Map(ILoggingEvent event) {
		...
		if (event.hasCallerData() && (callerData = event.getCallerData()[0]) != null) {
			item.put(Constant.LOG_FIELD_NAME_CLASS_NAME, callerData.getClassName());
			item.put(Constant.LOG_FIELD_NAME_FILE_NAME, callerData.getFileName());
			item.put(Constant.LOG_FIELD_NAME_LINE_NUMBER, callerData.getLineNumber());
			item.put(Constant.LOG_FIELD_NAME_METHOD_NAME, callerData.getMethodName());
		}
		...
		item.put(Constant.LOG_FIELD_NAME_LOG_MILLS, new Date(event.getTimeStamp()));
		/*
		 * build log info
		 */
